...
public class ArrayTableData<T> extends TableData<T> {
    ...
    public static<T> ArrayTableData<T> create(String tableName,String[] titleNames, T[][] data, boolean fix, int fixColumn, IDrawFormat<T> drawFormat){
        List<Column<T>> columns = new ArrayList<>();
        for(int i = 0;i <data.length;i++){
            T[] dataArray = data[i];
            Column<T> column = new Column<>(titleNames == null?"":titleNames[i], null,drawFormat);
            if (i==fixColumn) column.setFixed(fix);
            column.setDatas(Arrays.asList(dataArray));
            columns.add(column);
        }
        ArrayList<T> arrayList = new ArrayList<>(Arrays.asList(data[0]));
    ...
    public static<T> ArrayTableData<T> create(String tableName,String[] titleNames, T[][] data, boolean fix, int fixColumn, IFormat<T> format, IDrawFormat<T> drawFormat){
        List<Column<T>> columns = new ArrayList<>();
        for(int i = 0;i <data.length;i++){
            T[] dataArray = data[i];
            Column<T> column = new Column<>(titleNames == null?"":titleNames[i], null,format, drawFormat);
            if (i==fixColumn) column.setFixed(fix);
            column.setDatas(Arrays.asList(dataArray));
            columns.add(column);
        }
     ...
     * @param columns 列
     */
    public ArrayTableData(String tableName, List<T> t, List<Column<T>> columns) {
